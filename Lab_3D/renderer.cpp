...
Intersect Sphere::ray_intersect(V3 orig, V3 direction)
{
    ...
    long double d2 = pow(l, 2) - pow(tca, 2);
    Intersect intersect;
    if (d2 > pow(radius, 2))
    {
...
void Renderer::renderBlue()
{
    ...
    for (int y = 0, counter = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++, counter += 3)
        {
            ...
            long double average = (color.b + color.g + color.r) / 3;

            if (average != 255)
            {
                pixels[counter] = (unsigned char)average;
                pixels[counter + 1] = (unsigned char)0;
                pixels[counter + 2] = (unsigned char)0;
            }
        }
    }
}

void Renderer::renderRed()
{
    long double fov = (PI / 2);
    for (int y = 0, counter = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++, counter += 3)
        {
            ...
            {
                pixels[counter + 1] = (unsigned char)0;
                pixels[counter + 2] = (unsigned char)average;
            }
        }
    }
}