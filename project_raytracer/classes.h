...
public:
    V3 center;
    long double diagonal, center_normal;
    // Faces
    // Material faces[6];
    Material material;
    V3 min, max;

    // Cube where all the faces are the same
    // Cube(V3 center_arg, float diagonal_arg, Material universal_face)
    // {
    //     center = center_arg;
    //     diagonal = diagonal_arg;
    //     center_normal = diagonal / pow(2, 0.5);
    //     material = universal_face;
    //     // for (int i = 0; i < 6; i++)
    //     // {
    //     //     faces[i] = universal_face;
    //     // }
    // }
    Cube(V3 vmin, V3 vmax, Material universal_face)
    {
        min = vmin;
        max = vmax;
        material = universal_face;
    }
    // Cube where the faces might differ from each other
    // Cube(V3 center_arg, float diagonal_arg, Material face_arg[6])
    // {
    //     center = center_arg;
    //     diagonal = diagonal_arg;
    //     center_normal = diagonal / pow(2, 0.5);
...
public:
    ...
    Renderer(int x, int y, Cube scene_arg[], Light light_arg, int sceneSize_arg, V3 camera1)
    {
        width = x;
        height = y;
